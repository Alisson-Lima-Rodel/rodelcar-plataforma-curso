...
export const ENTITIES: Record<EntityKey, EntitySchema> = {
  ...
  courses: {
    ...
    search: (it) => `${it.titulo} ${it.sistema}`,
    filter: { key: "status", options: ["Todos", "Publicado", "Rascunho"] },
    columns: [
      { key: "titulo", label: "Curso", kind: "strong" },
      { key: "sistema", label: "Sistema", kind: "badgeSystem" },
      { key: "nivel", label: "Nível" },
      { key: "aulas", label: "Aulas", kind: "center" },
    ...
    fields: [
      ...
      { key: "aulas", label: "Nº de aulas", type: "number" },
      {
        key: "status",
        label: "Publicado",
        type: "toggle",
